...
{
#ifdef CONFIG_RISCV_S_MODE
	/* mhartid is M-mode only; S-mode cannot read it. Under SMP the boot
	 * code records each hart's id in its per-CPU struct, so read it back
	 * from there; otherwise the only hart running is the boot hart.
	 */
#ifdef CONFIG_SMP
