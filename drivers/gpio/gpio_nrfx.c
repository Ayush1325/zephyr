...
static int gpio_nrfx_port_get_regs(const struct device *port, gpio_raw_regs_t *regs)
{
	const struct gpio_nrfx_cfg *cfg = get_port_cfg(port);

	regs->set = &cfg->port->OUTSET;
	regs->clr = &cfg->port->OUTCLR;
...
#ifdef CONFIG_GPIO_GET_REGS_SET_CLR
	.port_get_regs = gpio_nrfx_port_get_regs,
#endif /* CONFIG_GPIO_GET_REGS_SET_CLR */
#ifdef CONFIG_GPIO_NRFX_INTERRUPT
