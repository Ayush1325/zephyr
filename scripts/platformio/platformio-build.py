...
stage0_ldscript = get_linkerscript_cmd(
    ...
    project_settings,
    extra_flags=["-DLINKER_DEVICE_HANDLES_PASS1"],
)

stage1_ldscript = get_linkerscript_cmd(
    app_includes,
    "linker_zephyr_pre1.cmd",
    project_settings,
    extra_flags=["-DLINKER_ZEPHYR_PREBUILT"],
)
...
for ldscript in (stage0_ldscript, stage1_ldscript, final_ld_script):
    env.Depends(ldscript, relocation_files)
    env.Depends(ldscript, offset_header_file)

env.Depends(stage1_ldscript, stage0_ldscript)
env.Depends(final_ld_script, stage1_ldscript)
...
if project_settings.get("CONFIG_HAS_DTS", ""):
    dev_handles = generate_dev_handles(stage0_elf_path)

stage1_elf_path = os.path.join("$BUILD_DIR", "zephyr", "firmware-pre1.elf")
env.Depends(stage1_elf_path, stage1_ldscript)

isr_table_file = generate_isr_table_file_cmd(stage1_elf_path, board, project_settings)
...
env.Append(
    ...
    _EXTRA_ZEPHYR_PIOBUILDFILES_PRE1=compile_source_files(
        target_configs["zephyr_pre1"], env, PROJECT_SRC_DIR
    ),
...
linker_arguments = extract_link_args(stage0_config)
